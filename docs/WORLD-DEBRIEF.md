...
3. **Selfie Check lives in the World ID App, not World App.** The credential page says "users are redirected to World ID App" and "anyone with World ID App can complete the flow"; our phone had only World App, whose "Get verified" offers Orb only, so every selfie attempt dead-ended. Nothing in the IDKit integration guide says which app answers which credential. Once known, the fix was one line: `.preset(selfieCheck())` with `allow_legacy_proofs: false`. We then installed the World ID App on the test phone (Android, 27 Sep 2026 04:00 JST): it opened with "beta, please come back soon" and Selfie Check could not be started. So the "unavailable credential" path is not hypothetical for us; the page shows the request, the app declines, and `ship` stays blocked.
4. **`world_id_4_not_available` reads like a version problem.** On our test phone (World App 4.0.4100) it meant "no credential at all"; "Get verified" offered only Orb, so no Selfie Check was possible for us. The error should say what is missing.
5. **Selfie Check legacy is a preview.** `selfieCheckLegacy()` is marked "contact us"; on our phone the selfie flow ran and then nothing came back. The page waited 7 minutes.
6. **Nullifier semantics in 4.0.** The docs say nullifiers are one-time-use and `session_id` is the stable link; for our "same human as before" check we bind the nullifier from the standard uniqueness proof and it has been stable across the simulator's proofs. A worked example of "recognise a returning user" without Session Proofs would remove the doubt.
